...
const QUIZ_QUESTIONS = [
  {
    q: "You want to estimate a qubit's state to high precision. A student suggests measuring the same qubit 10,000 times. What is wrong with this plan?",
    options: [
      "A qubit can only be measured about 100 times before it degrades and stops working",
      "Measuring a qubit gives one bit and locks the state permanently. You need 10,000 separately prepared qubits, each measured once",
      "Nothing — measuring the same qubit many times, each drawing from the original state, is exactly how statistics are built",
      "Re-preparing the qubit each time changes the state, making all results invalid"
    ],
    answer: 1,
    feedback: [
      "There is no such degradation limit. The issue is more fundamental: measurement permanently changes the qubit's state on the first measurement.",
      "Correct. After a single measurement, the qubit locks into whichever result it gave — that result now has a 100% chance. Every subsequent measurement just confirms that locked result. Your 10,000 measurements would tell you nothing about the original state.",
      "This is the loaded-die assumption — that the qubit is like a die you can roll many times. A qubit is not like that. The state changes permanently on the first measurement.",
      "Re-preparing to the same specification is exactly right. Each qubit is built to the same specification and measured once. That is the correct procedure, not a problem."
    ]
  },
  {
    q: "You run 10,000 measurements on two qubits: one in state (0.7071, 0.7071), one in state (\u22120.7071, 0.7071). What do the results look like?",
    options: [
      "The negative-amplitude qubit gives result 1 much more often — the minus sign pushes towards 1",
      "Both give results near 50/50 — squaring removes the sign, so both have identical chances",
      "The negative-amplitude qubit gives result 0 much more often — the minus sign pushes towards 0",
      "The negative amplitude makes the state unmeasurable until re-prepared with positive amplitudes"
    ],
    answer: 1,
    feedback: [
      "The sign does not affect the chance. The chance is the amplitude squared, and squaring removes the minus: (\u22120.7071)² = 0.5, the same as 0.7071² = 0.5.",
      "Correct. Both amplitudes square to 0.5, so both states give a 50/50 split. No amount of measurement can separate them. The difference is real and invisible — module five is about what it takes to reveal it.",
      "The minus sign on the first amplitude does not favour result 0. (\u22120.7071)² = 0.5, exactly the same chance as before.",
      "Negative amplitudes are entirely legal. The qubit measures normally."
    ]
  },
  {
    q: "After 10,000 measurements you see roughly 50% result 0 and 50% result 1. What can you conclude about the qubit's state?",
    options: [
      "The state is (0.7071, 0.7071) — that is the only pair that gives exactly 50/50",
      "The state has a 50/50 split, but could be (0.7071, 0.7071) or (\u22120.7071, 0.7071) — measurement cannot tell them apart",
      "The state is (0.5, 0.5), since those numbers directly represent 50% each",
      "You cannot draw firm conclusions from 10,000 shots — you would need at least a million to be sure"
    ],
    answer: 1,
    feedback: [
      "Both (0.7071, 0.7071) and (\u22120.7071, 0.7071) give 50/50 results, since squaring removes the sign. Measurement cannot distinguish them.",
      "Correct. Squaring removes the minus sign, so both states are identical from the perspective of measurement. The difference is real — it matters in other contexts, as module five shows — but measurement alone cannot reveal it.",
      "(0.5, 0.5) is not a legal state: 0.5² + 0.5² = 0.5, not 1. And even if it were, the deeper point stands: measurement cannot recover the sign.",
      "10,000 shots is more than enough to see a 50/50 split. The limitation here is not statistical. Even with a trillion shots, the sign is invisible to measurement."
    ]
  }
];
